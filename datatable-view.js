...
var DataTableView = Backbone.View.extend({
	...
	// properties and controls for this view's components
	'columnFilters':null,
	'modalForm':null,
	...
	'events':{
		...
		'init.dt':function(e, settings, json) {
			...
			// create column visibility controller
			// TODO need to be able to set group button set visibility from the constructor
			this.columnVisibilityControl = new DatatableColumnVisibilityControl({
				'class':'pull-left',
				'widthOverride':430,
				'columns':_.object(
					_.rest(this.datatable.columns().dataSrc().toArray(),1), 
					_.map(
				...
				'groups':this.model.get('groups')?this.model.get('groups'):[]
			});
			$('.column-visibility-container', dt).append(this.columnVisibilityControl.render());
			
			// column visibility event listeners
			// when the DatatableView triggers a "column-visibility-change" event it will send 
			...
			this.listenTo(this.columnVisibilityControl, 'column-visibility-change', function(columnIndex, isVisible) {
				//console.log('column-visibility-change handled: '+ columnIndex + ', ' + isVisible);
				this.datatable.column(columnIndex).visible(isVisible, false);
				this.datatable.columns.adjust().draw( false );
				this.trigger('column-visibility-change', 
					this.datatable.columns(':visible:not(.action-column)').indexes().toArray(),
					this.datatable.columns(':visible:not(.action-column)').dataSrc().toArray()
				);
			});
			this.listenTo(this.columnVisibilityControl, 'group-visibility-change', function(visibleColumnIndexes) {
				//console.log('group-visibility-change handled: ' + visibleColumnIndexes.join(','));
				// first hide all columns except the first
				this.datatable.columns(':not(.action-column)').visible(false, false);
				
				// show columns using visibleColumnIndexes
				this.datatable.columns(visibleColumnIndexes).visible(true, false);
				this.datatable.columns.adjust().draw( false );
				this.trigger('column-visibility-change', 
		...
		'click .column-visibility-container ul.dropdown-menu input, .column-visibility-container ul.dropdown-menu label':function(e) {
			console.log('==CLICK== EVENT FOR COLUMN VISIBILITY CONTROL');
			e.stopPropagation();
		},
		'change .column-visibility-container ul.dropdown-menu input':function(e) {
			console.log('==CHANGE== EVENT FOR COLUMN VISIBILITY CONTROL');
			this.datatable.columns(e.currentTarget.value*1).visible(e.currentTarget.checked);
			e.stopPropagation();
			return false;
		},
	...
	'initialize':function(options) {
		...
		if(_.has(options.table, 'groups')) {
			this.model.set('groups', options.table.groups);
		}
		...
		*/
		...
		var primCol = _.findWhere(options.table.columns, {'data':options.table.primaryKey}),
			...
			defaultDatepickerOpts = {'autoclose':true, 'format':'m/d/yyyy'},
			datatableview = this;
			...
			if(col.data != options.table.primaryKey) {
				...
				if(_.has(col,'render')) {
					...
					_.extend(metaCol, {'render': function(data,type,full,meta) {
						return _.isFinite(data) ? moment(new Date(data)).format('M/D/YYYY') : '';
					}});
				}
